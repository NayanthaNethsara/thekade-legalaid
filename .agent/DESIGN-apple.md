...
description: A photography-first interface that turns marketing into a museum gallery. Edge-to-edge product tiles alternate light and dark canvases, framed by SF Pro Display headlines with negative letter-spacing and a single Violet Accent (#7c3aed) interactive color. UI chrome recedes so the product can speak — no decorative gradients, no shadows on chrome, only the one signature drop-shadow under product imagery resting on a surface.
...
- Single violet accent (`{colors.primary}` — #7c3aed) carries every interactive element. No second brand color exists.
...
### Brand & Accent
- **Violet Accent** (`{colors.primary}` — #7c3aed): The single brand-level interactive color. All text links, all violet pill CTAs ("Learn more", "Buy"), and the focus ring root. This is Apple's quiet but universal "click me" signal. Press state shifts to a slightly darker variant via the active scale transform rather than a hex change.
- **Focus Violet** (`{colors.primary-focus}` — #8b5cf6): A marginally brighter sibling of Violet Accent, reserved for the keyboard focus ring on buttons (`outline: 2px solid`).
- **Sky Violet** (`{colors.primary-on-dark}` — #a78bfa): A brighter violet used on dark surfaces for in-copy links and inline callouts, where Violet Accent would disappear against the tile background.
...
**`button-primary`** — The signature Apple action. Background `{colors.primary}` (Violet Accent #7c3aed), text `{colors.on-primary}` in `{typography.body}` (SF Pro Text 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
- Active state: `{component.button-primary-active}` — `transform: scale(0.95)` (the system-wide micro-interaction).
- Focus state: `{component.button-primary-focus}` — 2px solid `{colors.primary-focus}` outline.

**`button-secondary-pill`** — Used as the second CTA when two violet pills appear together ("Learn more" / "Buy"). Background transparent, text `{colors.primary}`, 1px solid `{colors.primary}` border, rounded `{rounded.pill}`, padding 11px × 22px. Reads as a "ghost pill."
...
**`button-store-hero`** — A larger primary CTA used on store hero surfaces. Same Violet Accent + Paper White as `{component.button-primary}`, but with `{typography.button-large}` (18px / 300 — note the rare weight 300) and slightly more padding (14px × 28px). Used sparingly on the store landing.
...
**`text-link`** — Inline body links in `{colors.primary}` (Violet Accent). Underlined or non-underlined per context.

**`text-link-on-dark`** — Inline body links on dark tiles in `{colors.primary-on-dark}` (Sky Violet #a78bfa) — Violet Accent would disappear against `{colors.surface-tile-1}`.
...
### Do
- Use `{colors.primary}` (Violet Accent #7c3aed) for every interactive element — links, pill CTAs, focus signals — and nothing else. The single accent is non-negotiable.
