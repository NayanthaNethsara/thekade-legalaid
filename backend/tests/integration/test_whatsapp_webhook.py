...
class TestWhatsAppWebhook:
    ...
    async def test_receive_message_text(self, client, sample_whatsapp_message):
        ...
        with patch(
            "app.services.message_processor.MessageProcessor.process_message"
        ) as mock_process:
    ...
    async def test_receive_message_voice(self, client, sample_voice_message):
        ...
        with patch(
            ...
            response = client.post("/webhook", json=sample_voice_message)
    ...
    @pytest.mark.asyncio
    async def test_receive_message_processing_error(
        self, client, sample_whatsapp_message
    ):
        ...
        with patch(
            ...
            # The current implementation doesn't catch exceptions, so it will
            # raise the exception. We expect this to result in a 500 status code
            try:
                client.post("/webhook", json=sample_whatsapp_message)
                # If we get here, the test failed because it should have raised
                # an exception
    ...
    def test_webhook_get_method_requires_params(self, client):
        ...
        response = client.get(
            "/webhook",
            params={"verify_token": "test_verify_token", "challenge": "1234567890"},
        )
        assert response.status_code == 422

        # Test missing verify_token
        response = client.get(
            "/webhook", params={"mode": "subscribe", "challenge": "1234567890"}
        )
        assert response.status_code == 422
        ...
        response = client.get(
            "/webhook",
            params={"mode": "subscribe", "verify_token": "test_verify_token"},
        )
        assert response.status_code == 422
