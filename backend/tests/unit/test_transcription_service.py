...
class TestTranscriptionService:
    ...
    def test_transcribe_success(
        self, mock_model, transcription_service, test_audio_file
    ):
    ...
    @patch("app.services.transcription_service.model")
    def test_transcribe_with_noise(
        self, mock_model, transcription_service, test_audio_file
    ):
    ...
    @patch("app.services.transcription_service.model")
    def test_transcribe_long_audio(
        self, mock_model, transcription_service, test_audio_file
    ):
    ...
    @patch("app.services.transcription_service.model")
    def test_transcribe_multilingual(
        self, mock_model, transcription_service, test_audio_file
    ):
    ...
    @patch("app.services.transcription_service.model")
    def test_transcribe_with_special_characters(
        self, mock_model, transcription_service, test_audio_file
    ):
        """Test transcription containing special characters."""
        # Mock transcription with special characters
        mock_result = {
            "text": ("Legal case #123: Smith vs. Jones (2024) - " "$50,000 settlement.")
        }
        mock_model.transcribe.return_value = mock_result

        result = transcription_service.transcribe(test_audio_file)

        expected = "Legal case #123: Smith vs. Jones (2024) - $50,000 settlement."
    ...
    @patch("app.services.transcription_service.model")
    def test_transcribe_legal_terminology(
        self, mock_model, transcription_service, test_audio_file
    ):
        """Test transcription of legal terminology."""
        # Mock transcription with legal terms
        legal_text = (
            "According to the Motor Traffic Act, section 52, "
